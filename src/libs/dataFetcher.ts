import { Category, categorySchema, Post, postSchema } from '@/types/entry';
...
export async function fetchCategory(slug: string): Promise<Category> {
  ...
  const categoryFilePath = path.join('_data', '_categories', `${slug}.md`);
...
export async function fetchCategories(): Promise<Category[]> {
  const categoryFilePaths = await glob(
    path.join('_data', '_categories', '*.md')
  );
...
export async function fetchPost(slug: string): Promise<Post> {
  ...
  const postFilePath = path.join('_data', '_posts', `${slug}.md`);
...
export async function fetchAllPost(): Promise<Post[]> {
  const categoryFilePaths = await glob(path.join('_data', '_posts', '*.md'));
