...
アメリカ英語、イギリス英語の違いというのはわかりました。ではなんでこの2つの表記が存在しているのでしょうか。
...
おそらく現代ではこの感覚、逆なのでは？と私は思います（あくまで私が）。おそらくですが、当時は最先端のスキルを持ついわゆる「オタク」（て言い方を変えるなら「ギーク」）達の流行は長音符を付けないことだったのだと思われます。
...
実は、長音符を省き続けていた、JIS規格さんですが、[JIS Z8301:2019](https://kikakurui.com/z8/Z8301-2019-01.html)ではそのルールを変更しました。

> H.6 外来語の表記 
> 外来語の表記は，主として“外来語の表記（平成3.6.28 内閣告示第2号）”による。

この内閣告示第2号というのは、上記にも登場しているルールであり、これは `-er`, `-or`, `-ar` などに長音符をつけるルールも含まれています。
...
もちろん数多くの例外があります。メモリ・プロセッサ・トランジスタ・レジスタ・コンデンサ・エンジニア。日本全国に普及していますが、未だにUSBメモリでありUSBメモリーではないですよね（なんとなく、USBメモリーは見たことある気もします）。
