...
この方法でデジタル署名を生成することが出来ます。DSAの方法とは異なり、デジタル署名から平文を生成することができるのがRSAの特徴と呼べます。実際にはRSAでデジタル署名を行う場合は、実際はメッセージのハッシュ(SHA2などを利用する)を取得し、そのハッシュに対してRSAで署名を作成する方法が一般的かと思われます。
